...
import type { EmployeePublic, ApiResponse, PaginatedResponse } from "@business-profile/shared";
...
export default function AdminDashboard() {
  ...
  const fetchEmployees = useCallback(async () => {
    try {
      const res = await clientApiFetch<PaginatedResponse<EmployeePublic>>("/employees");
      setEmployees(res.data || []);
      setTotal(res.total);
    } catch {
      // handled silently — auth redirect will catch 401
    } finally {
      setLoading(false);
    }
  }, []);
    ...
    <div className="min-h-screen bg-surface">
      <header className="border-b">
        <div className="max-w-6xl mx-auto px-6 h-14 flex items-center justify-between">
          <p className="text-sm font-semibold tracking-wide uppercase text-ink-tertiary">
            Dashboard
          </p>
      ...
      <main className="max-w-6xl mx-auto px-6 py-10">
        <div className="flex items-end justify-between mb-8">
          ...
          <Link
            ...
            className="px-4 py-2 bg-accent text-surface-raised text-sm font-semibold rounded-lg hover:bg-accent-hover transition-colors"
          >
            Add employee
          </Link>
        </div>
          ...
          <div className="overflow-hidden rounded-xl border">
            <table className="w-full">
              ...
              <tbody className="divide-y">
                {employees.map((emp) => (
                  <tr key={emp.id} className="hover:bg-accent-subtle transition-colors">
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-ink-tertiary">{emp.email}</p>
                        </div>
                      </div>
                    </td>
